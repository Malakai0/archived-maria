...
local DIRECTION_CHANGE_MULTIPLIER = 0.02
local DEFAULT_MULTIPLIER = 0.04
...
function ODM:_calculatePhysics()
	...
	local Multiplier = PrioritizedIf({
		DEFAULT_MULTIPLIER,
		self._directionChange ~= nil,
		DIRECTION_CHANGE_MULTIPLIER,
		self.Boosting == true,
		BOOST_MULTIPLIER,
	})
	...
	local MaxForce = Lerp(LinearVelocity.MaxForce, MAX_BV_FORCE, 0.1)
	local Velocity

	if LinearVelocity.VectorVelocity.Unit:Dot(Direction) >= 0.85 then
...
function ODM:_update(dt)
	...
	local TiltSpeed = if NoHooks
		then 0
		else self.LinearVelocity.VectorVelocity.Magnitude / (self.Properties.MaxSpeed * 2)
...
function ODM:_calculateDriftingVelocity(BaseVelocity, Matrix, Target, Distance, Speed, Multiplier)
	...
	return BaseVelocity:Lerp(CFrame.new(RootP, Facing.Position).LookVector * Speed, Multiplier)
end
